...
@section('title', 'Tambah Kandidat - E-Voting SMAN 4')
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 sm:p-8">
        <div class="mb-6">
            <h1 class="text-lg font-bold text-slate-900">Form Pendaftaran Kandidat</h1>
            <p class="text-xs text-slate-500">Lengkapi data profil calon pengurus OSIS/MPK</p>
        </div>
        ...
        <form action="{{ route('admin.candidates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="candidate_number" id="candidate_number" value="{{ old('candidate_number', 1) }}" min="1" max="99" required
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Muhammad Rizky Pratama"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
            ...
            <div>
                ...
                <div class="flex items-start space-x-5">
                    <div id="previewContainer" class="w-24 h-32 rounded-xl bg-slate-100 border border-dashed border-slate-300 overflow-hidden flex items-center justify-center flex-shrink-0 relative">
                        <img id="photoPreview" src="" alt="Preview" class="w-full h-full object-cover hidden">
                        <span id="previewPlaceholder" class="text-[10px] text-slate-400 text-center px-2">Preview Foto</span>
                    </div>
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-[11px] text-slate-400 mt-1.5">Gunakan pasfoto seragam sekolah dengan latar polos untuk tampilan surat suara yang profesional.</p>
            ...
            <div>
                ...
                <textarea name="vision" id="vision" rows="3" placeholder="Tuliskan gagasan utama atau visi kandidat..."
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ old('vision') }}</textarea>
            ...
            <div>
                ...
                <textarea name="mission" id="mission" rows="4" placeholder="Tuliskan butir-butir misi yang akan dilaksanakan (misal: 1. ... 2. ...)"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ old('mission') }}</textarea>
